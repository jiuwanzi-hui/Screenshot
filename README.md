...
- **在线或完全离线的多语言翻译**：设置中可选择关闭翻译、使用 OpenAI 兼容的在线大模型，或使用本机 Bergamot 模型。两种模式都会自动识别截图文字的源语言，不再把 OCR 识别语言误当作翻译源语言；离线模式使用本机 CLD3 对每个 OCR 分段检测语言，并支持 Mozilla 当前模型清单里的 58 种源语言和 53 种目标语言。日文转中文等非英语组合会自动按“源语言 → English → 目标语言”中转。离线下载按目标语言打包，覆盖全部受支持源语言；确认前会实时显示本次下载流量、新增安装占用、磁盘余量和实际安装目录。文件会校验并解压到 `SnapCut.exe` 旁的 `TranslationModels`，语言检测和翻译过程都不上传文字。在线模式仍支持获取厂家模型列表或手动输入模型标识，API Key 使用当前用户 DPAPI 加密保存。两种模式都能批量翻译 OCR 分段、切换原文/译文并把译文覆盖到最终图片，整组译文可一步撤销和重做。
...
- [安装版 SnapCut Setup 2.3.0（GitHub）](https://github.com/jiuwanzi-hui/Screenshot/releases/latest/download/SnapCut-Setup-2.3.0-win-x64.exe)
- [免安装版 SnapCut Portable 2.3.0（GitHub）](https://github.com/jiuwanzi-hui/Screenshot/releases/latest/download/SnapCut-Portable-2.3.0-win-x64.zip)
- [安装版 SnapCut Setup 2.3.0（Gitee 国内镜像）](https://gitee.com/wwangyunhui/screenshot/releases/download/v2.3.0/SnapCut-Setup-2.3.0-win-x64.exe)
- [免安装版 SnapCut Portable 2.3.0（Gitee 国内镜像）](https://gitee.com/wwangyunhui/screenshot/releases/download/v2.3.0/SnapCut-Portable-2.3.0-win-x64.zip)

免安装版解压后直接运行 `SnapCut.exe`，不需要安装或预先配置 .NET。安装版和免安装版的数据都保存在各自程序目录的 `ScreenshotData` 中；移动或卸载程序前请按需备份该目录。2.3.0 延续现有数据布局，并会在发现 1.0.0 的 `%LocalAppData%\Screenshot` 旧数据时尝试迁移到新位置，迁移成功后删除旧目录。
...
- **Online or fully offline multilingual translation**: Settings can disable translation, use an OpenAI-compatible online model, or use local Bergamot models. Both modes detect the language of the captured text automatically instead of treating the selected OCR language as the translation source. Offline mode runs CLD3 locally for each OCR segment and supports the 58 source languages and 53 target languages currently present in Mozilla's model registry. Non-English pairs such as Japanese to Chinese are routed through English. Offline downloads are packaged by target language and cover all supported source languages. Before downloading, SnapCut calculates and displays the actual transfer, additional installed size, free disk space, and destination. Files are verified and extracted to `TranslationModels` beside `SnapCut.exe`; neither detection nor translation uploads text in offline mode. Online mode still supports vendor model discovery or a manually entered model id, with API keys protected by per-user Windows DPAPI. Both modes batch-translate OCR regions, switch between source and translated text, and include the reversible translated overlay in copied or saved images.
...
- [SnapCut Setup 2.3.0 (GitHub)](https://github.com/jiuwanzi-hui/Screenshot/releases/latest/download/SnapCut-Setup-2.3.0-win-x64.exe)
- [SnapCut Portable 2.3.0 (GitHub)](https://github.com/jiuwanzi-hui/Screenshot/releases/latest/download/SnapCut-Portable-2.3.0-win-x64.zip)
- [SnapCut Setup 2.3.0 (Gitee China mirror)](https://gitee.com/wwangyunhui/screenshot/releases/download/v2.3.0/SnapCut-Setup-2.3.0-win-x64.exe)
- [SnapCut Portable 2.3.0 (Gitee China mirror)](https://gitee.com/wwangyunhui/screenshot/releases/download/v2.3.0/SnapCut-Portable-2.3.0-win-x64.zip)

Extract the portable archive and run `SnapCut.exe`; neither installation nor a preinstalled .NET runtime is required. Both packages store their data in `ScreenshotData` under their respective application directories, so back up that directory before moving or uninstalling the application. Version 2.3.0 retains the existing data layout and attempts to migrate legacy 1.0.0 `%LocalAppData%\Screenshot` data when found, removing the old directory after a successful migration.
