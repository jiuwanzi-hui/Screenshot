...
public sealed class ScrollCaptureFastScrollTests
{
    ...
    public void MatchesLargeSingleFrameDisplacements(double displacementRatio)
    {
        ...
        var document = CreateDocumentContent(width, height + newRows + 32);
        var previousFrame = Crop(document, 0, height);
        var currentFrame = Crop(document, newRows, height);
    ...
    public void StitchesAnAcceleratingFlingWithoutGapsOrDuplicates()
    {
        ...
        var document = CreateDocumentContent(width, height + tops[^1]);
        using var composer = new ScrollCaptureComposer();

        foreach (var top in tops)
        {
            var frame = Crop(document, top, height);
            Assert.True(
    ...
    public void KeepsFollowingTheViewportWhenTheWheelEstimateIsFarTooSmall()
    {
        ...
        var document = CreateDocumentContent(width, height + newRows + 32);
        using var composer = new ScrollCaptureComposer();
        var firstFrame = Crop(document, 0, height);
        var secondFrame = Crop(document, newRows, height);
    ...
    public void StitchesMixedSpeedScrollingWithCreepsPausesAndBursts()
    {
        ...
        var document = CreateDocumentContent(
            width,
            height + steps.Sum());
        using var composer = new ScrollCaptureComposer();

        Assert.True(composer.TryAddFrame(
            Crop(document, 0, height),
            FastOptions,
            out _));
        ...
        int documentTop = 0)
    {
        ...
        for (var y = 0; y < result.Height; y += 3)
        {
            ...
            {
                Assert.Equal(
                    GetPixel(document, x, y + documentTop),
                    GetPixel(result, x, y));
            }
        }
    }
    ...
    /// </remarks>
    private static PixelImage CreateDocumentContent(int width, int height)
    {
        var content = new PixelImage(width, height);
        Fill(content, Rgb(250, 250, 248));
        var bandColor = Rgb(232, 236, 244);
        ...
        for (var line = 0; line * lineHeight < height; line++)
        {
            ...
            if (line % 17 == 0)
            {
                FillRect(content, 0, top + 4, width, 12, bandColor);
            }
            ...
            while (x < limit)
            {
                ...
                for (var glyph = 0; glyph < wordLength; glyph++)
                {
                    ...
                    var glyphColor = Rgb(
                        (byte)shade,
                        (byte)shade,
                        (byte)Math.Min(255, shade + 8));
                    var edgeColor = Rgb(
                        (byte)Math.Min(255, shade + 90),
                        (byte)Math.Min(255, shade + 90),
                        (byte)Math.Min(255, shade + 98));
                    FillRect(content, glyphLeft, top + 6, 7, 11, glyphColor);
                    FillRect(content, glyphLeft, top + 6, 7, 1, edgeColor);
                    FillRect(content, glyphLeft + 6, top + 6, 1, 11, edgeColor);
                }
